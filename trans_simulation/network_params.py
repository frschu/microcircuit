...
class net:
    def __init__(self, g=4., j02=1., C_ab=None, PSC_rel_sd=0.0):
        ...
        self.populations        = np.array([layer + typus for layer in layers for typus in types])
        ...
        self.n_neurons_micro  = np.int_(full_scale_n_neurons * self.area)
        self.n_total    = np.sum(self.n_neurons_micro)
        ...
        self.n_neurons_equal  = np.tile([N_exc, N_inh], self.n_layers).astype(int)
        ...
        # Scale synapse numbers of the model
        if self.scale_K_linearly:
            ...
            n_outer         = np.outer(self.n_neurons_micro, self.n_neurons_micro)
            K_scaled        = np.int_(np.log(1. - conn_probs   ) / np.log(1. - 1. / n_outer))
        self.C_ab_micro = (K_scaled / self.n_neurons_micro)

        if C_ab == None:
            C_e     = np.mean(self.C_ab_micro) # mean for microcircuit (= 501 in full scale)
            C_i     = gamma * C_e
            self.C_ab   = np.tile([C_e, C_i], (self.n_populations, self.n_layers)) # depends only on pre
        else:
            self.C_ab   = C_ab

        ###################################################
        ###          Further network: not so central    ###        
        ###################################################
        ...
        self.delays  = np.tile([delay_e, delay_i], (len(populations), len(layers))) # adapt...
