...
# Raster:
#n_neurons_rec_spike = (model.n_neurons * 0.025).astype(int)
#n_neurons_rec_voltage = (model.n_neurons * 0.00).astype(int)
#t_sim   =   400.0 # ms
#t_trans =   100.0 # ms
# Else: record from 1000 neurons
...
with h5py.File(path_hdf5_file, "w") as data_file:
    # Attributes
    data_file.attrs["area"]     = model.area
    data_file.attrs["t_sim"]    = t_sim*1e-3
    data_file.attrs["t_trans"]  = t_trans*1e-3
