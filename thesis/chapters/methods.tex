...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=1.0\linewidth]{\figdir syn_numbers}
    \caption{
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=1.0\linewidth]{\figdir syn_numbers_distribution}
    \caption{
        Distribution of synapse numbers for the connection $L4e \to L4i$. 
...
$L5i$, the other one to population $L6e$. Potjans and Diesmann state that
these two populations have among the highest / lowest rates of the simulation\cite{potjans2014}.
This is reflected in the average membrane potential which for the neuron of $L5i$
is much closer to the threshold $\theta = -50$ mV, whereas for the $L5e$ neuron, it is
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=1.0\linewidth]{\figdir single_membrane_potential}
    \caption{
        Membrane potentials of two exemplary neurons of the populations 
        $L5i$ (\textbf{A, C}) and $L6e$ (\textbf{B, D}). The data is taken from 
        ...
        two neurons (steps) as well as the averaged ones for a subset of $100$ neurons 
        (dashed colored lines). Note that contributions due to neurons being in refractory 
...
is ommitted to facilitate readability, as the results are only concerned with stationary
...
\subsection{Numerics for mean field theory}
The coupled integrals of equation \eqref{eq:self_consistency_a} are solved numerically. 
The functions used are taken from the numpy-scipy environment \cite{scipy} 
for python \cite{python}.
The numerical implementation of $\text{erf}(u)$ has a limited domain for the applied 
...
The analysis of the data is done in using python \cite{python} and heavily relies upon 
the numpy-scipy framework \cite{scipy}. Plots are created using matplotlib \cite{matplotlib}, 
colors are chosen based on ColorBrewer\cite{brewercolor}.

Analyzing the membrane potentials of simulated neurons requires a specific treatment 
