...
\begin{equation}
    ...
    \label{eq:t_max}
\end{equation}
Inserting into \autoref{eq:V(t)} leads to 
\begin{equation}
    V(t_\text{max}) 
...
stable under a wide range of conditions (\marginpar{CITE}). 
...
thalamic signals (\marginpar{CITE}). The model implemented in this work is only driven 
...
\begin{figure}[tb]
    ...
    \caption[Connection between populations]{
        ...
        rule "fixed indegree". %(\marginpar{see section comparison for details}).
    }
    \label{fig:syn_numbers}
\end{figure}
\paragraph{In order to verify} that the model behaves reasonably, we examine a number of basic parameters 
...
To further validate the model on a basic level, we take a look at a single neurons' membrane potentials 
...
%\marginpar{Under cortical condition: (Amit, Brunel); 
...
Inserting both $P_0(V)$ and $p_{r, 0}$ into the self-consistent condition, \autoref{eq:P_V_prob}
...
As done by \citeb{brunel2000}, this equation can be solved numerically for varying parameters to 
characterize the different states the system can be found in.

\paragraph{What has been introduced}so far serves as a basis for the theory newly derived for this
...
following the early work of \citeb{amit1997model}. 
...
\marginpar{
I'm not sure how well defined taking the integral of $\delta^2$ actually is...
}
\begin{equation}
    \begin{split}
...
Including both the variance of the synaptic weights as well as the adaption 
...
With these parameters, we turn back to the self-consistency condition 
...
values in neocortical neurons is given by \citeb{nawrot2008measurement}, who report values
...
ISI distribution (see e.\,g. \citeb{van2010stochastic} for an overview). In principle,
...
A solution to \autoref{eq:self_consistency_a} is not necessarily found for any initial guess. 
...
%\marginpar{a much broader range} 
