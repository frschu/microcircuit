...
\headerbox{MOTIVATION}{name=motivation, column=0, span=2}{
    \begin{multicols}{2}
    Current research on the function and activity of the neocortex relies heavily
    on numerical simulations of spiking network models. Recent models include one
    of the outstanding anatomical features of the neocortex: its arrangement in 
    various layers. Although they are able to 
    reproduce experimental measures, it is often unclear what
    specific mechanisms are important for the network activity and how sensitive the 
    model is to changes in parameters. The presented mean field model is one 
    way to tackle these aspects: It predicts the network activity assuming 
    uncorrelated input to each neuron and can be utilized as an 
    effective tool for scanning large portions of the parameter space. 
    \end{multicols}
}
...
\headerbox{SPIKING NETWORK MODEL}{name=network, column=0, span=1, below=motivation}{
    ...
    experimental data by Potjans and Diesmann~\cite{potjans2014}. Synapses are drawn randomly until 
