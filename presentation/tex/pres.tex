...
\begin{frame}[J]{Central goals of the thesis}
    \begin{block}<1->
        {Implement spiking network model of the neocortex}
        \vspace{0.2cm}
        \footnotesize
    ...
    \vfill
    \begin{block}<2->
        {Develop mean field theory for the neocortical model}
        \vspace{0.2cm}
        \footnotesize
        Nicolas Brunel. \\
...
\begin{frame}[t]{Examples}
    ...
    \begin{column}[T]{5cm} % each column can also be its own environment
        \only<1->{
            Over time
            \includegraphics[height=0.7\textheight]{../figures/single_membrane_potential}
        }
    \end{column}
    \begin{column}[T]{5cm} % alternative top-align that's better for graphics
        \only<2->{
...
\begin{frame}[t]{Single neuron activity}
    \only<1->{
        \includegraphics[height=0.4\textheight]{../figures/single_neuron_activity_rates} \\
    }
    \only<2->{
        \includegraphics[height=0.4\textheight]{../figures/single_neuron_activity_irregularity}
    }
\end{frame}
