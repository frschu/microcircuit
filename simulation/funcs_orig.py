...
# Functions
#######################################################
def prepare_simulation(master_seed, n_populations, data_path):
    ...
    if sim.to_text_file:
        data_path_text = os.path.join(data_path, "text_files")
        if not os.path.exists(data_path_text):
            os.makedirs(data_path_text)
        nest.SetKernelStatus({"data_path": data_path_text})
    ...
    sli_str += r"% global RNG " + "\n"
    sli_str += r"/rng_seeds "
    sli_str += "%i [0 %i 1 sub] add Range "%(master_seed, sim.n_vp)
    sli_str += r"% local RNG seeds " + "\n"
    sli_str += r"/grng_seed "
    sli_str += "%i %i add "%(master_seed, sim.n_vp)
    sli_str += r"% global RNG seed " + "\n"
    sli_str += ">> SetStatus"
...
def create_nodes(model, pyrngs):
    ...
    neuron_GIDs = np.array([range(*boundary) for boundary in zip(lower_GIDs, upper_GIDs + 1)])
    ...
    if sim.record_cortical_spikes:
        spike_detector_dict = [{"label": sim.spike_detector_label + population + "_", 
                                "to_file": sim.to_text_file} for population in model.populations]
